...
class SOAP(optim.Optimizer):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if group["correct_bias"]:
                    ...
                    step_size = step_size * \
                        (bias_correction2 ** .5) / bias_correction1
    ...
    def init_preconditioner(self, grad, state, precondition_frequency=10,
        ...
        # Will hold all the eigenbases of the preconditioner.
        state['Q'] = None
    ...
    def update_preconditioner(self, grad, state,
        ...
        if grad.dim() == 1:
            if precondition_1d and grad.shape[0] <= max_precond_dim:
                state['GG'][0].lerp_(grad.unsqueeze(
                    1) @ grad.unsqueeze(0), 1-state['shampoo_beta'])
        else:
            if merge_dims:
    ...
    def get_orthogonal_matrix_QR(self, state, max_precond_dim=10000, merge_dims=False):
        ...
        if merge_dims:
            if self._data_format == 'channels_last' and len(orig_shape) == 4:
                exp_avg_sq = exp_avg_sq.reshape(
                    permuted_shape).permute(0, 2, 3, 1)
            else:
